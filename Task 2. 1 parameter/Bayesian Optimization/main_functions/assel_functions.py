...
def analyse_simulation(E_laser,filtered,obj_type):
    ...
    # Задача 2: конвертация энергии до 100% ---> maximize it
    elif obj_type == 3:
    ...
    # desy's objective function ---> maximize it
    elif obj_type == 4:
        
        Energy_beam = [] # different calculation of E_beam (don't know which is correct)
        
        # Beam energy (total energy of all electrons) (desy calculation)
        Energy_beam_init = 20
        ...
        obj_func = E_conv[-2]
        print(f'Energy conversion = {E_conv[0]} %')
...
def best_sim_obj(varied_param,filtered,obj_type):
