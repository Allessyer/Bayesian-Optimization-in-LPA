...
def best_sim_analysis(path2sim,best_sim,input_params_names,from_iter,till_iter):
...
#     params_convergence = give_param_convergence(first_N_simulations,sorted_dict_obj,input_params_names,path2sim)
    
#     for param in params_convergence.keys():
...
# def plot_results(path2diags,input_params_names,from_iter=9):
...
#     for iteration in range(from_iter,10):
#         z,uz = ts.get_particle(var_list = ['z','uz'],species='bunch',iteration=iteration)
#         above150_uz = []
#         above150_z = []
#         below150_uz = []
...
#         fig, (ax1,ax2) = plt.subplots(figsize=(7,5),nrows=2, sharex=True, subplot_kw=dict(frameon=False)) # frameon=False removes frames

#         plt.subplots_adjust(hspace=.0)
#         ax1.grid()
#         ax2.grid()

#         ax1.plot(above150_z,above150_uz,'.',ms=0.5,color='orange', label = 'above 150 MeV')
#         ax1.plot(below150_z,below150_uz,'.',ms=0.5,color='#beab9e',label = 'below 150 MeV')
#         ax1.legend()

#         ax2.plot(m.z, Ez[Ez.shape[0]//2,:],color='#723505')


#         ax1.set_ylabel('uz [$m_e \cdot c$]')
#         ax2.set_ylabel('Ez [MeV]')
#         ax1.set_title(f"Iteration N {iteration}")

#         plt.xlabel('z [m]')
#         ax1.set_xlim([m.zmin, m.zmax])
#         ax2.set_xlim([m.zmin, m.zmax])
...
#         plt.imshow(F, extent=m.imshow_extent, aspect='auto')
#         plt.clim(-1.e11,1.e11)
#         # plt.imshow(F, extent=m.imshow_extent, vmin=-.1e7, aspect='auto')
#         plt.colorbar()
